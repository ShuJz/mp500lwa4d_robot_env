...
CPHIDGETINIT(TemperatureSensor)
	...
	switch(phid->phid.deviceIDSpec)
	{
		...
		{
			int tries = 20;
			//make sure temperature are known - can take a few reads for the sensor to start returning data.
			while((phid->AmbientTemperature == PUNK_DBL || phid->Temperature[0] == PUNK_DBL) && tries--)
				CPhidget_read((CPhidgetHandle)phid);
			if(phid->AmbientTemperature == PUNK_DBL || phid->Temperature[0] == PUNK_DBL)
				LOG(PHIDGET_LOG_WARNING, "Couldn't read 1045 temperature during init - some value will be unknown.");
			break;
		}
		default:
			break;
...
CPHIDGETDATA(TemperatureSensor)
	...
	switch(phid->phid.deviceIDSpec)
	{
		...
		case PHIDID_TEMPERATURESENSOR_IR:
			if(phid->phid.deviceVersion >= 100 && phid->phid.deviceVersion < 200) 
			{
				//If we get an 0xFFFF value, just ignore it.

				//Temp IC - absolute possible range: -61.1 - 161.16
				if(buffer[3] == 0xff && buffer[2] == 0xff)
					AmbientTemperature = PUNK_DBL;
				else
	...
	if(AmbientTemperature == PUNK_DBL && phid->phid.deviceIDSpec == PHIDID_TEMPERATURESENSOR_IR){
		LOG(PHIDGET_LOG_INFO, "Ignoring 0xFFFF value for Ambient Sensor on 1045");
	}
	else if(AmbientTemperature > phid->ambientTemperatureMax)
	{
		phid->AmbientTemperature = PUNK_DBL;
	...
	for (i = 0; i<phid->phid.attr.temperaturesensor.numTempInputs; i++)
	{
		//If ambient temperature was outside of allowed range, then calculated temperature will be wrong (for thermocouples)
		if(phid->AmbientTemperature == PUNK_DBL
			&& (phid->phid.deviceIDSpec == PHIDID_TEMPERATURESENSOR || phid->phid.deviceIDSpec == PHIDID_TEMPERATURESENSOR_4))
		{
			phid->Temperature[i] = PUNK_DBL;
		...
		else if(Temperature[i] == PUNK_DBL)
		{
			switch(phid->phid.deviceIDSpec) {
				...
				case PHIDID_TEMPERATURESENSOR_4:
					...
					break;
				case PHIDID_TEMPERATURESENSOR_IR:
					LOG(PHIDGET_LOG_INFO, "Ignoring 0xFFFF value for IR Temperature Sensor on 1045");
				default:
					break;
			}
		}
