...
//NOTE: used to disable the read during firmware upgrade reboot, because it stalls forever of M3 devices with the USB stack bug on El Capitan
int CCONV
CPhidget_disableRead(CPhidgetHandle phid) {
	TESTPTR(phid)
	phid->disableRead = PTRUE;
	return EPHIDGET_OK;
}
