...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound, ACTION_NOISE, ON_TRAIN):
    ...
    def choose_action(self, s):
        if self.ON_TRAIN:
            return self.sess.run(self.a, {self.S: s[np.newaxis, :], self.tf_is_training: True})[0]
        else:
